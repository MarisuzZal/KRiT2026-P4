...
# Pusty string też idzie do default (np. gdy SDE_INSTALL=$SDE_INSTALL w shellu)
SDE_INSTALL = os.environ.get("SDE_INSTALL", "") or \
...
_paths = [
    ...
    # Niektóre wersje SDE umieszczają bfrt_grpc w site-packages/tofino/bfrt_grpc
    f"{SDE_INSTALL}/lib/{_PY}/site-packages/tofino/bfrt_grpc",
]
...
if not os.path.isdir(SDE_INSTALL):
    print(f"[sde_paths] OSTRZEŻENIE: SDE_INSTALL='{SDE_INSTALL}' nie istnieje. "
          ...
          file=sys.stderr)
    print(f"[sde_paths] Spróbuj: export SDE_INSTALL=$SDE/install   "
          f"(i sprawdź czy $SDE ustawione w shell-u).",
          file=sys.stderr)
